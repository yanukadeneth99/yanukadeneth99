...
// Languages excluded from the donut. These bloat the byte counts but aren't
// what people mean by "what does this person code in":
//   - Jupyter Notebook: each .ipynb is JSON wrapping the actual Python plus
//     base64-encoded image outputs — a single notebook can dwarf an entire
//     TypeScript codebase by raw bytes
//   - HTML/CSS: often generated by static-site builders, framework scaffolds,
//     or vendored UI kits — not authorship signal
//   - Markdown / TeX / Roff: documentation, not code
      ...
      .map(async (repo) => {
        try {
          ...
          for (const [lang, bytes] of Object.entries(data || {})) {
            // Skip non-authorship languages before they pollute the chart.
            // Filtering at aggregation time means percentages are computed
            // over real-code bytes only — the donut now shows TS as 50%
            // instead of 7% because Jupyter doesn't eat 85% of the pie.
